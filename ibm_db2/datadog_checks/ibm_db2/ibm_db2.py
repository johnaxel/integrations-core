...
class IbmDb2Check(AgentCheck):
    ...
    def __init__(self, name, init_config, instances):
        ...
        self._db = instance.get('db', '')
        self._username = instance.get('username', '')
        self._password = instance.get('password', '')
        self._host = instance.get('host', '')
        self._port = instance.get('port', 5000)
    ...
    def query_instance(self):
        statement = ibm_db.exec_immediate(self._conn, queries.INSTANCE_TABLE)
    ...
    def query_database(self):
        ...
        self.gauge(self.m('lock.pages'), result['lock_list_in_use'] / 4096, tags=self._tags)
        ...
        self.monotonic_count(self.m('row.modified.total'), result['rows_modified'], tags=self._tags)

        # https://www.ibm.com/support/knowledgecenter/SSEPGG_11.1.0/com.ibm.db2.luw.admin.mon.doc/doc/r0001317.html
        self.monotonic_count(self.m('row.reads.total'), result['rows_read'], tags=self._tags)

        # https://www.ibm.com/support/knowledgecenter/SSEPGG_11.1.0/com.ibm.db2.luw.admin.mon.doc/doc/r0051569.html
        self.monotonic_count(self.m('row.returned.total'), result['rows_returned'], tags=self._tags)
    ...
    def query_buffer_pool(self):
        ...
        statement = ibm_db.exec_immediate(self._conn, queries.BUFFER_POOL_TABLE)
    ...
    def query_table_space(self):
        ...
        statement = ibm_db.exec_immediate(self._conn, queries.TABLE_SPACE_TABLE)
    ...
    def query_transaction_log(self):
        statement = ibm_db.exec_immediate(self._conn, queries.TRANSACTION_LOG_TABLE)
        result = ibm_db.fetch_assoc(statement)
        ...
        self.monotonic_count(self.m('log.reads'), result['log_reads'], tags=self._tags)

        # https://www.ibm.com/support/knowledgecenter/SSEPGG_11.1.0/com.ibm.db2.luw.admin.mon.doc/doc/r0001279.html
        self.monotonic_count(self.m('log.writes'), result['log_writes'], tags=self._tags)
